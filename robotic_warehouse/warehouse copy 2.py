...
class Warehouse(gym.Env):
    ...
    def reset(self):
        ...
        # Logging initial positions
        print("Initial agent positions:", [(agent.x, agent.y) for agent in self.agents])
        print("Initial shelf positions:", [(shelf.x, shelf.y) for shelf in self.shelfs])
        print("Initial request queue:", [(shelf.x, shelf.y) for shelf in self.request_queue])
    ...
    def step(self, actions: List[Action]) -> Tuple[List[np.ndarray], List[float], List[bool], Dict]:
        ...
        for agent in self.agents:
            ...
            if agent.req_action == Action.FORWARD:
                ...
                if shelf_id:
                    ...
                    logger.info(f"Agent {agent.id} picked up shelf {shelf_id} at position {(agent.x, agent.y)}")
            elif agent.req_action == Action.TOGGLE_LOAD and agent.carrying_shelf:
                if not self._is_highway(agent.x, agent.y):
                    logger.info(f"Agent {agent.id} dropped shelf {agent.carrying_shelf.id} at position {(agent.x, agent.y)}")
        ...
        for x, y in self.goals:
            ...
            logger.info(f"Shelf {shelf_id} delivered by Agent {agent_id} to goal at {(x, y)}")
        ...
        logger.info("Agent positions: %s", [(agent.x, agent.y) for agent in self.agents])
